...
class BaseRLPlayer(SimpleRLPlayer, ABC):
	...
	@abstractmethod
	def policy(self, state, i_episode: int = 0, eps_greedy: bool = True):
		pass
...
class CombineActionRLPlayer(BaseRLPlayer):
	def __init__(
		...
		**kwargs
		...
		self.pareto_thresh = pareto_thresh
		self._init_model(input_size, hidden_layers)
	...
	def policy(self, state, i_episode: int = 0, eps_greedy: bool = True) -> int:
		self.policy_net.eval()
		sample = random.random()
		eps_threshold = self.get_eps_threshold(i_episode)
		self.eps_threshold = eps_threshold
		...
		if not eps_greedy or sample > eps_threshold:
			...
			if (
				random.random() < self.pareto_p
				and thresh <= self.pareto_thresh
