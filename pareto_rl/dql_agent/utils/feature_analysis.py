...
def sample_transitions(player: BaseRLPlayer, num_episodes: int, file_name: str, **args):
    ...
    data = {"labels": obs_labels, "observations": observations, "rewards": rewards}
    with open("".join(["./feature_selection/", file_name, ".pickle"]), "wb") as handle:
        pickle.dump(data, handle)

    obs_labels.append("rewards")
    observations = [
        obs + [reward] for obs, reward in zip(data["observations"], data["rewards"])
    ]
...
def pca(file_name: str, n_components: int, n_most_corr: int):
    with open("".join(["./feature_selection/", file_name, ".pickle"]), "rb") as handle:
        ...
        components = [
            {"var_ratio": var_ratio, "top_k_names": [], "top_k_corr": []}
        ...
        for i, correlations in enumerate(pca.components_):
            ...
            sorted_corr, labels = zip(
                *sorted(zip(abs_corr, data["labels"]), reverse=True)
            )
            components[i]["top_k_corr"] = sorted_corr[:n_most_corr]
            components[i]["top_k_names"] = labels[:n_most_corr]
            for label, corr in zip(
                components[i]["top_k_names"], components[i]["top_k_corr"]
            ):
        ...
        feature_scores = {"names": names, "scores": scores}
        ...
        df.to_csv(
            "".join(
                [
                    ...
                    file_name,
                    "_pca",
                    f"_{n_components}pc_{n_most_corr}corr",
                    ".csv",
                ]
            )
        )
        ...
        df.to_csv(
            "".join(
                ["./feature_selection/", file_name, "_pca", "_feature_scores", ".csv"]
            )
        )
...
def sfs(file_name: str):
    with open("".join(["./feature_selection/", file_name, ".pickle"]), "rb") as handle:
        ...
        sfs = SequentialFeatureSelector(
            kmeans, n_features_to_select=20, tol=None, n_jobs=8
        )
        sfs.fit(df)
