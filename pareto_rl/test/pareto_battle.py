...
def main(args):
    ...
    if not args.dry:
        ...
        for i, file in enumerate(files):
            ...
            plot_utils.generate_multilevel_diagram(
                population, f"NSGA2 run {i} multilevel diagram"
            )
            plot_utils.plot_results_multi_objective_PF(
                population, f"NSGA2 run {i} objective vs objective", args
            )
        plot_utils.generate_multilevel_diagram_different_population(
            populations, "NSGA2 runs comparisons multilevel diagram"
        )
